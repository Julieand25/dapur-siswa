<x-admin-layout active="laporan-rekod" title="Rekod Peralatan & Barang" subtitle="Rekod penggunaan peralatan dan barang masakan oleh pengguna.">
    ...
    <main class="content">
        ...
        <div class="table-card">
            ...
            <form class="table-toolbar" method="GET" action="{{ route('laporan.rekod') }}">
            ...
            @if ($reviews->isEmpty())
                <div class="empty-msg">Tiada rekod dijumpai.</div>
            @else
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        @foreach ($reviews as $r)
                            <tr>
                                ...
                                <td>{{ \Carbon\Carbon::parse($r->created_at)->timezone('Asia/Kuala_Lumpur')->format('d/m/Y') }}</td>
                                <td>{{ $r->nama }}</td>
                                <td>{{ $r->matrik }}</td>
                                <td>{{ $r->emel }}</td>
                                <td>{{ $r->location_code }}</td>
                                <td>{{ $r->kitchen_name }}</td>
                                <td>{{ $r->bilangan_hidangan ?? '—' }}</td>
                                <td>
                                    <a href="{{ route('laporan.rekod.show', $r->id) }}" class="btn-icon" title="Lihat">
                ...
                @if ($reviews->hasPages())
                    <div class="pagination">
                        <span class="pag-info">Memaparkan {{ $reviews->firstItem() }} hingga {{ $reviews->lastItem() }} daripada {{ $reviews->total() }} rekod</span>
                        <div class="pag-pages">
                            @if (! $reviews->onFirstPage())
                                <a href="{{ $reviews->previousPageUrl() }}" class="pag-btn"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg></a>
                            @endif
                            @foreach ($reviews->getUrlRange(1, $reviews->lastPage()) as $page => $url)
                                <a href="{{ $url }}" class="pag-btn {{ $page == $reviews->currentPage() ? 'active' : '' }}">{{ $page }}</a>
                            @endforeach
                            @if ($reviews->hasMorePages())
                                <a href="{{ $reviews->nextPageUrl() }}" class="pag-btn"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg></a>
                            @endif
                        </div>
                    </div>
                @endif
