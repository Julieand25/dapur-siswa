<x-admin-layout active="laporan-maklumbalas" title="Maklum Balas Pengguna" subtitle="Senarai maklum balas daripada pengguna sistem.">
    ...
    <style>
        ...
        .search-wrap input {
            ...
            color: #374151; outline: none; transition: border-color 0.15s; background: #fff;
        }
        ...
        .search-wrap svg {
            ...
            transform: translateY(-50%); width: 15px; height: 15px; color: #9ca3af;
        }

        .filter-select {
            padding: 9px 28px 9px 10px; border: 1.5px solid #d1d5db; border-radius: 6px;
        ...
        tbody td:first-child { font-weight: 600; color: #1a3a8f; }
        ...
        .komen-text { max-width: 220px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

        .empty-msg { text-align: center; padding: 40px 20px; color: #9ca3af; font-size: 13px; }
        ...
        @media (max-width: 640px) { .content { padding: 16px; } .komen-text { max-width: 120px; } }
    </style>
    @endpush
    ...
    <main class="content">
        ...
        <div class="table-card">
            ...
            <form class="table-toolbar" method="GET" action="{{ route('laporan.maklumbalas') }}">
                <div class="search-wrap">
                    ...
                    <input type="text" name="search" placeholder="Cari nama atau no. matrik..." value="{{ request('search') }}">
                </div>
            ...
            @if ($feedbacks->isEmpty())
                <div class="empty-msg">Tiada maklum balas dijumpai.</div>
            @else
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        @foreach ($feedbacks as $f)
                            <tr>
                                ...
                                <td>{{ $f->nama }}</td>
                                <td>{{ $f->matrik }}</td>
                                <td>{{ \Carbon\Carbon::parse($f->created_at)->timezone('Asia/Kuala_Lumpur')->format('d/m/Y') }}</td>
                                <td><span class="stars" data-rating="{{ $f->keseluruhan }}"></span></td>
                                <td><span class="komen-text">{{ \Illuminate\Support\Str::limit($f->komen, 60) }}</span></td>
                                <td>
                                    <a href="{{ route('laporan.maklumbalas.show', $f->id) }}" class="btn-icon" title="Lihat">
                ...
                @if ($feedbacks->hasPages())
                    <div class="pagination">
                        <span class="pag-info">Memaparkan {{ $feedbacks->firstItem() }} hingga {{ $feedbacks->lastItem() }} daripada {{ $feedbacks->total() }} maklum balas</span>
                        <div class="pag-pages">
                            @if (! $feedbacks->onFirstPage())
                                <a href="{{ $feedbacks->previousPageUrl() }}" class="pag-btn"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg></a>
                            @endif
                            @foreach ($feedbacks->getUrlRange(1, $feedbacks->lastPage()) as $page => $url)
                                <a href="{{ $url }}" class="pag-btn {{ $page == $feedbacks->currentPage() ? 'active' : '' }}">{{ $page }}</a>
                            @endforeach
                            @if ($feedbacks->hasMorePages())
                                <a href="{{ $feedbacks->nextPageUrl() }}" class="pag-btn"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg></a>
                            @endif
                        </div>
                    </div>
                @endif
